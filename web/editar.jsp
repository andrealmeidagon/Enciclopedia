...
<div class="container">
    <div class="span12">
        <table class="table table-striped">
            <thead>
                <tr><th><a href="UsuarioController?ref=listarinst" class="btn btn-success"><i class="icon-white icon-plus"></i> Adicionar Administrador</a></th></tr>
                <tr>
                    <th></th>
                    <th>Nome</th>
                    <th>Instituição</th>
            ...
            <tbody>
                <%
                    if(request.getAttribute("bdUsuario")!=null && request.getAttribute("bdInst")!=null){
                        ArrayList<Usuario> arAdmin = (ArrayList<Usuario>) request.getAttribute("bdUsuario");
                        ArrayList<Instituicao> arInst = (ArrayList<Instituicao>) request.getAttribute("bdInst");
                        for(int i=0; i<arAdmin.size(); i++){
                            out.print("<tr>");
                            out.print("<td><a href='UsuarioController?ref=editar&id="+arAdmin.get(i).getId_usuario()+"' class='btn btn-primary btn-xs'> Editar</a> <title> </title><a href='AdminController?ref=excluir&id="+arAdmin.get(i).getId_usuario()+"' class='btn btn-danger btn-xs'><i class='icon-remove'></i></a></td>");
                            out.print("<td>"+arAdmin.get(i).getNom_usuario()+"</td>");
                            for(int j=0; j<arInst.size(); j++){
                                if(arAdmin.get(i).getInstituicao_id()==arInst.get(j).getId_instituicao())
                                    out.print("<td>"+arInst.get(j).getSigla_instituicao()+"</td>");
                            }
                            out.print("<td>"+arAdmin.get(i).getEmail_usuario()+"</td>");
                            out.print("</tr>");
                        }
                    }
                %>
